...
router.post('/api/students/login', async (req, res) => {
	...
	const query = "SELECT `password`  from college.students WHERE id= '" + id + "'";
	console.log(query);
	...
	const tokenQuery = "UPDATE college.students SET `token` = '" + token + "'  WHERE id= '" + id + "'";
	...
	connection.query(query2, function(error, results, fields) {
		if (error || !compare) res.status(401).send({ error: 'Authetication Failed' });
		else {
			results[0] = {
				...results[0],
				token,
...
router.get('/api/students', async (req, res) => {
	const connection = mysql.createConnection(coursesDB);
	connection.query('SELECT * from college.students', function(error, results, fields) {
		if (error) res.send(error);
		else {
...
router.patch('/api/students', auth, authPassword, async (req, res) => {
	...
	const updates = Object.keys(req.body).filter(
		update => update === 'phone' || update === 'password' || update === 'residence'
	);
	console.log(updates);
	...
	updates.forEach(update => {
		query = 'UPDATE `college`.`students` SET `' + update + "` = '" + req.body[update] + "' WHERE id= '" + id + "';";
		console.log(query);
		connection.query(query, function(error, results, fields) {
			if (error) res.send(error);
			else res.send();
